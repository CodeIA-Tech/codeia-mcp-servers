#!/usr/bin/env node
/**
 * Script para atualizar a mensagem do step Message_Status_Tomcat no workflow "MVP - Automation"
 * Uso: node scripts/update-message-status-tomcat-workflow.js
 */
...
function updateMessageStatusTomcatStep(workflowData) {
  ...
  // Encontrar o step Message_Status_Tomcat
  const messageStatusStep = steps.find(step => step.name === 'Message_Status_Tomcat');
  
  if (!messageStatusStep) {
    throw new Error('Step Message_Status_Tomcat não encontrado no workflow');
  }

  // Criar nova mensagem amigável no mesmo formato do Info_Message
  // A mensagem anterior era: "Instancias: {{ Steps.Generate_detailed_message.data }}\n\nServiço reiniciado\n{{ Steps.GetCommandInvocation.command }}"
  // Vamos melhorar para incluir informações sobre o status do comando
  const newMessage = `✅ **Status da Reinicialização do Serviço Tomcat**
...
{{ Steps.Get_Command_Invocation.command }}
...
_Esta mensagem foi gerada automaticamente pelo workflow MVP - Automation_`;
  ...
  const updatedStep = JSON.parse(JSON.stringify(messageStatusStep));
  ...
  const updatedSteps = steps.map(step => {
    if (step.name === 'Message_Status_Tomcat') {
      return updatedStep;
    }
    return step;
  });
  ...
  return { updatedSpec, oldMessage: messageStatusStep.parameters.find(p => p.name === 'message')?.value || '', newMessage };
}
...
async function main() {
  try {
    ...
    console.log('🛠️  Atualizando mensagem do step Message_Status_Tomcat...');
    const { updatedSpec, oldMessage, newMessage } = updateMessageStatusTomcatStep(workflowDetails);
    ...
    console.log(`   • Step atualizado: Message_Status_Tomcat`);
    console.log(`   • Instância: {{ Steps.Generate_Detailed_Message.data }}`);
    console.log(`   • Horário: {{ Workflow.startedAt }}`);
    console.log(`   • Status: Inclui informações do comando executado`);
    console.log('');
    console.log('🔗 Links:');
